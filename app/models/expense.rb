...
# 	end 
  ...
  class << self
    def spent(type)
      ...
      if type == 'today'
        ...
        self.select(:created_at, :cost).where(trip_id: current_user.trips.id, created_at: (today).at_beginning_of_day..today.at_end_of_day).each { |e| total += e.cost}        
      elsif type == 'week'
        self.select(:created_at, :cost).where(created_at: (today - 7).at_beginning_of_day..today.at_end_of_day).each { |e| total += e.cost}
        #self.select("created_at AS date, SUM(cost)").where("created_at BETWEEN ? AND ?", (today - 7).at_beginning_of_day, today.at_end_of_day)
      else type == 'month'
        self.select(:created_at, :cost).where(trip_id: current_user.trips.id, created_at: (today - 30).at_beginning_of_day..today.at_end_of_day).each { |e| total += e.cost}
  ...
  class << self
    def balance(type)
      if type == 'today'
        self.trips.daily_budget - spent("today")
      elsif type == 'week'
        (self.trips.daily_budget * 7) - spent("week")
      else type == 'month'
        (self.trips.daily_budget * 30) - spent("month")
      end
    end
  end
